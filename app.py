...
def run():
    ...
    if choice == 'Normal User':
        st.markdown('''<h5 style='text-align: left; color: white;'> Upload your resume, and get smart recommendations</h5>''',
        ...
        if st.button('Login'):
            if ad_user == 'bharathi' and ad_password == 'Bharu@1234':
                ...
                # Assuming `values` is supposed to be the counts of unique predicted fields
                values = plot_data['Predicted_Field'].value_counts()
                print(values)
                # Ensure `values` and `labels` have the same length
                labels = values.index
                print(labels)
                ...
                # Similarly for User's Experienced Level
